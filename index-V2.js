...
//* Search -------------------------------------------------------------------->
const search = function(stations, searchId) {
  const index = stations.findIndex(function(station, index) {
    return station.station_id === searchId;
  });
...
function bikesTotal() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#docks-disabled > p").html("<strong>" + totalDocksDisabled + "</strong><br>espacios bloqueados");
...
function bikesStation() {
  ...
  console.log("searchValue del input: " + searchValue);
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      const search = function(stations, id) {
        const index = stations.findIndex(function(station, index) {
          return station.name === searchValue;
        });
        return stations[index];
      };
      ...
      if (typeof result !== "undefined") {
        searchId = result.station_id;
        console.log("searchId: " + searchId);

        searchName = result.name;
        console.log("searchName: " + searchName);

        searchNumber = searchName.slice(0, 3);
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          const search = function(stations, id) {
            const index = stations.findIndex(function(station, index) {
              return station.station_id === searchValue;
            });
            return stations[index];
          };
          ...
          var stationDocksDisabled = result.num_docks_disabled;
          ...
          $("#docks-disabled > p").html("<strong>" + stationDocksDisabled + "</strong><br>espacios bloqueados");
